...
/// Set by the host application (tokcat's `native_tray::init`) once it has
/// installed its own CALayer onto the status item's button. While this is
/// true, this crate's set_icon paths become no-ops — the host owns what
/// gets rendered into the menubar.
static TAKEOVER: AtomicBool = AtomicBool::new(false);

pub fn set_takeover(v: bool) {
    TAKEOVER.store(v, Ordering::SeqCst);
}
...
use objc2_core_graphics::{
    ...
    CGDataProviderCreateWithData, CGDisplayPixelsHigh, CGImageAlphaInfo, CGImageCreate,
    CGMainDisplayID,
};
...
fn set_icon_for_ns_status_item_button(
    ...
    if let Some(icon) = icon {
        ...
        let rgba: Vec<u8> = icon.inner.rgba_bytes().to_vec();

        let icon_height: f64 = 18.0;
        let icon_width: f64 = (width as f64) / (height as f64 / icon_height);

        // Build NSImage directly from raw RGBA via a CGImage. This avoids the
        // RGBA → PNG encode + PNG → RGBA decode round-trip that NSImage's
        // initWithData: path forces, which was costing real CPU when animating
        // the tray icon (PNG codecs invoked through libPng/ImageIO every swap).
        unsafe {
            // Move the RGBA buffer into a Box<Vec<u8>>; CoreGraphics keeps it
            // alive via the data provider's release callback below.
            let boxed: Box<Vec<u8>> = Box::new(rgba);
            let bytes_len = boxed.len();
            let data_ptr = boxed.as_ptr() as *const c_void;
            let info_ptr = Box::into_raw(boxed) as *mut c_void;

            unsafe extern "C-unwind" fn release_data(
                info: *mut c_void,
                _data: NonNull<c_void>,
                _size: usize,
            ) {
                let _ = unsafe { Box::from_raw(info as *mut Vec<u8>) };
            }
            ...
            .expect("CGImageCreate failed for tray icon RGBA buffer");

            let new_size = NSSize::new(icon_width, icon_height);
            let nsimage =
                NSImage::initWithCGImage_size(NSImage::alloc(), &cgimage, new_size);
